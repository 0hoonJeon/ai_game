...
st.write("static 폴더 존재 여부:", os.path.exists("static"))
st.write("파일 존재 여부:", os.path.exists("static/tetris.html"))

# 텍스트로 파일을 읽어서 내용이 정상적으로 불러와지는지 확인
try:
    with open("static/tetris.html", "r", encoding="utf-8") as f:
        content = f.read()
        st.write("파일 길이:", len(content))
except Exception as e:
    st.error(f"파일을 읽을 수 없음: {e}")
